...
func (v *numericValidator) generate(out *codegen.Emitter, format string) error {
	...
	if v.isNillable && v.nullable {
		checkValue = fmt.Sprintf("(%s.IsSpecified() && !%s.IsNull())", value, value)
		getValue = fmt.Sprintf("%s.MustGet()", value)
	} else if v.isNillable {
		checkValue = fmt.Sprintf("%s != nil", value)
		getValue = fmt.Sprintf("*%s", value)
	}
	...
	if v.multipleOf != nil {
		if v.roundToInt {
			if v.isNillable {
				...
				out.Printlnf(`if %s %% %v != 0 {`, getValue, v.valueOf(*v.multipleOf))
			}
			out.Indent(1)
			...
			if v.isNillable {
				out.Printlnf(`if %s {`, checkValue)
			} else {
				out.Printlnf("{")
			}

			out.Indent(1)
			out.Printlnf("remainder := math.Mod(%s, %v)", getValue, v.valueOf(*v.multipleOf))
			out.Printlnf(
